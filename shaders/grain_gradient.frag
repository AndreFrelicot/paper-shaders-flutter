...
float randomR(vec2 p) {
  ...
  float n = texture(u_noiseTexture, fract(uv)).r;
...
void main() {
  ...
  if (u_shape > 3.5) {
    ...
    grain_uv *= ps_objectBoxSize();
    grain_uv *= .7;
  } else {
    shape_uv = .5 * ps_patternUV();
    grain_uv = 100. * ps_patternUV();
    ...
    if (u_fit > 0.) {
      ...
      grain_uv /= (patternBoxNoFitBoxWidth / ps_patternBoxSize().x);
    }
    vec2 patternBoxScale = u_resolution.xy / ps_patternBoxSize();
  ...
  if (u_shape < 1.5) {
    ...
    // Upstream uses clamp(0., 1., length(...)). SwiftShader treats that
    // invalid min/max ordering as 1.0 in the golden harness.
    shape = 0.;
