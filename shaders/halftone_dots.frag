...
void main() {
  ...
  for (int xi = 0; xi < 6; xi++) {
    if (float(xi) >= stepMultiplier) continue;
    float x = -0.5 + float(xi) * stepSize;
    for (int yi = 0; yi < 6; yi++) {
      if (float(yi) >= stepMultiplier) continue;
      float y = -0.5 + float(yi) * stepSize;
