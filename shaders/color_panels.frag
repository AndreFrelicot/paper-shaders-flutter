...
void main() {
  ...
  for (int set = 0; set < 2; set++) {
    ...
    for (int i = 0; i <= 20; i++) {
      ...
      int colorIdx = int(glsl_mod(float(idx), float(colorsCount)));
      int nextColorIdx = int(glsl_mod(float(idx + 1), float(colorsCount)));

      vec4 colorA = cp_colorAt(colorIdx);
      vec4 colorB = cp_colorAt(nextColorIdx);
    ...
    for (int i = 0; i <= 20; i++) {
      ...
      int colorIdx = int(glsl_mod(float(colorsCount) - glsl_mod(float(idx), float(colorsCount)), float(colorsCount)));
      if (colorIdx < 0) colorIdx += colorsCount;
      int nextColorIdx = int(glsl_mod(float(colorIdx + 1), float(colorsCount)));
