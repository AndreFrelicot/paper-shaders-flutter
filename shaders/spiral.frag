...
float spiralOffsetAt(vec2 uv, float t) {
  float l = length(uv);
...
float spiralShapeAt(vec2 uv, float t) {
  float stripe = fract(spiralOffsetAt(uv, t));
  return 2. * abs(stripe - .5);
}

void main() {
  vec2 uv = 2. * ps_patternUV();
  ...
  vec2 px = 2. * ps_patternPixelStepX();
  vec2 py = 2. * ps_patternPixelStepY();
  float offsetX = spiralOffsetAt(uv + px, t);
  float offsetY = spiralOffsetAt(uv + py, t);
  float shapeX = spiralShapeAt(uv + px, t);
  float shapeY = spiralShapeAt(uv + py, t);

  float fw = ps_finiteFwidth(offset, offsetX, offsetY);
  float fwMult = 4. - 3. * (smoothstep(.05, .4, 2. * u_strokeWidth) * smoothstep(.05, .4, 2. * (1. - u_strokeWidth)));
  float pixelSize = mix(fwMult * fw, ps_finiteFwidth(shape, shapeX, shapeY), clamp(fw, 0., 1.));
