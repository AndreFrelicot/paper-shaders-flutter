...
float blurEdge3x3(vec2 uv, float radius, float centerSample) {
  vec2 texel = 1.0 / vec2(1750.0, 1499.0);
  vec2 r = radius * texel;
  ...
  sum += w2 * texture(u_image, uv + vec2(0.0, -r.y)).g;
  sum += w2 * texture(u_image, uv + vec2(0.0, r.y)).g;
  sum += w2 * texture(u_image, uv + vec2(-r.x, 0.0)).g;
  sum += w2 * texture(u_image, uv + vec2(r.x, 0.0)).g;

  sum += w1 * texture(u_image, uv + vec2(-r.x, -r.y)).g;
  sum += w1 * texture(u_image, uv + vec2(r.x, -r.y)).g;
  sum += w1 * texture(u_image, uv + vec2(-r.x, r.y)).g;
  sum += w1 * texture(u_image, uv + vec2(r.x, r.y)).g;
...
void main() {
  vec2 uv = ps_objectUV() + .5;
  uv.y = 1. - uv.y;

  vec2 imgUV = ps_imageUV();
  imgUV -= .5;
  ...
  img[1] = blurEdge3x3(imgUV, 8., img[1]);
