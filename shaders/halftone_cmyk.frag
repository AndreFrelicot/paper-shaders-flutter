...
void main() {
  ...
  vec2 grainUV = (ps_imageUV() - .5) * grainSize + .5;
  ...
  if (isJoined) {
    ...
    C = smoothstep(th - sLeft - ps_pixelDerivative(1.0), th + sRight, C);
    M = smoothstep(th - sLeft - ps_pixelDerivative(1.0), th + sRight, M);
    Y = smoothstep(th - sLeft - ps_pixelDerivative(1.0), th + sRight, Y);
    K = smoothstep(th - sLeft - ps_pixelDerivative(1.0), th + sRight, K);
  }
